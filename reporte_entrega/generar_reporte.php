...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enviar_reporte'])) {
    ...
    if (empty($nit) || empty($token_temporal)) {
        ...
        foreach ($reportes as $index => $reporte) {
            ...
            if ($curl_error) {
                ...
                try {
                    ...
                    ");
                    $stmt_prescripcion->execute([$reporte['ID']]);
                    $result_prescripcion = $stmt_prescripcion->fetch(PDO::FETCH_ASSOC);
                    $no_prescripcion_real = $result_prescripcion['no_prescripcion'] ?? '';
        ...
        if ($exitosos > 0) {
            registrar_log_actividad($pdo, $usuario['id'], 'GENERAR_REPORTE_ENTREGA', "Reportes enviados: {$exitosos} de " . count($reportes));
        }
