...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['consultar'])) {
    ...
    if (empty($nit) || empty($token_temporal) || empty($no_prescripcion)) {
        ...
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTAR_ENTREGA_ERROR', "Validación fallida: campos incompletos. Prescripción: {$no_prescripcion}");
    } else {
        // Procesar token: quitar "=" y agregar %3D al final
        $token_procesado = rtrim($token_temporal, '=');
        
        $apiResult = $mipresApiClient->get(
            "EntregaXPrescripcion/{$nit}/{$token_procesado}%3D/{$no_prescripcion}",
            30
        );
        ...
        if ($curl_error) {
            ...
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTAR_ENTREGA_ERROR', "Error cURL en consulta de entrega por prescripción {$no_prescripcion}: {$curl_error}");
        } elseif ($http_code === 200) {
            $mensaje = '✓ Se encontró una entrega registrada para esta prescripción';
            $tipo_mensaje = 'success';
            $respuesta_api = json_decode($response, true);
            $cantidad = is_array($respuesta_api)
                ? (isset($respuesta_api[0]) ? count($respuesta_api) : 1)
                : 0;
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTAR_ENTREGA', "Consulta exitosa - Prescripción: {$no_prescripcion}, Registros: {$cantidad}");
        } elseif ($http_code === 404) {
            $mensaje = 'No se encontró ninguna entrega registrada para el número de prescripción consultado';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTAR_ENTREGA_ERROR', "Sin registros (404) para prescripción {$no_prescripcion}");
        } elseif ($http_code === 401) {
            $mensaje = 'El token ha expirado o no es válido. Por favor genere un nuevo token temporal e intente nuevamente';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTAR_ENTREGA_ERROR', "Token inválido/expirado (401) en consulta de entrega. Prescripción: {$no_prescripcion}");
        } elseif ($http_code >= 500) {
            $mensaje = 'El servidor de MIPRES no está disponible en este momento. Por favor intente nuevamente en unos minutos';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTAR_ENTREGA_ERROR', "Error servidor MIPRES (HTTP {$http_code}) en consulta de entrega. Prescripción: {$no_prescripcion}");
        } else {
            $mensaje = 'Ocurrió un error al consultar la entrega. Si el problema persiste, contacte al soporte técnico (Código: ' . $http_code . ')';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTAR_ENTREGA_ERROR', "Error HTTP {$http_code} en consulta de entrega. Prescripción: {$no_prescripcion}");
        }
    }
}
