...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['consultar'])) {
    ...
    if (empty($nit) || empty($fecha) || empty($token_temporal) || empty($tipo_doc) || empty($numero_doc)) {
        $mensaje = 'Por favor complete todos los campos para realizar la consulta';
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_PROGRAMACION_PACIENTE_ERROR', "Validación fallida: campos incompletos. Paciente: {$tipo_doc}-{$numero_doc}, Fecha: {$fecha}");
    } else {
        // Procesar el token: quitar el "=" final y agregar %3D
        $token_procesado = rtrim($token_temporal, '=') . '%3D';
        
        $apiResult = $mipresApiClient->get(
            "ProgramacionXPacienteFecha/{$nit}/{$fecha}/{$token_procesado}/{$tipo_doc}/{$numero_doc}",
            15
        );
        ...
        if ($curl_error) {
            ...
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_PROGRAMACION_PACIENTE_ERROR', "Error cURL en consulta paciente {$tipo_doc}-{$numero_doc} fecha {$fecha}: {$curl_error}");
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                if (count($resultados) > 0) {
                    $mensaje = '✓ Consulta exitosa: Se encontraron ' . count($resultados) . ' registro(s) de programación para este paciente en la fecha seleccionada';
                    $tipo_mensaje = 'success';
                    registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_PROGRAMACION_PACIENTE', "Paciente: {$tipo_doc}-{$numero_doc}, Fecha: {$fecha}, Resultados: " . count($resultados));
                } else {
                    $mensaje = 'No se encontraron registros de programación para el paciente y fecha consultados';
                    $tipo_mensaje = 'warning';
                    registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_PROGRAMACION_PACIENTE', "Paciente: {$tipo_doc}-{$numero_doc}, Fecha: {$fecha}, Resultados: 0");
                }
            } else {
                $mensaje = 'La respuesta del servidor no tiene el formato esperado. Por favor intente nuevamente';
                $tipo_mensaje = 'error';
                registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_PROGRAMACION_PACIENTE_ERROR', "JSON inválido en consulta paciente {$tipo_doc}-{$numero_doc}. HTTP: {$http_code}");
            }
        } else {
            $mensaje = 'No se pudo completar la consulta. Si el problema persiste, contacte al soporte técnico (Código: ' . $http_code . ')';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_PROGRAMACION_PACIENTE_ERROR', "HTTP {$http_code} en consulta paciente {$tipo_doc}-{$numero_doc}, fecha {$fecha}");
        }
    }
}
...
<head>
    ...
    <title>Consultar Programación por Paciente - <?php echo APP_NAME; ?></title>
    ...
    <div class="layout">
        <aside class="sidebar">
            ...
            <div class="sidebar-section">
                ...
                <div class="sidebar-children open" id="menu-programacion">
                    ...
                    <a href="consultar_por_paciente.php" class="sidebar-item sidebar-child active">Consultar por Paciente</a>
                </div>
            </div>
	  ...
	  <div class="sidebar-section">
        ...
        <main class="main-content">
            ...
            <div class="card">
                <h2>Consultar Programación por Paciente</h2>
                <p class="card-subtitle">Consulta la programación de un paciente específico en una fecha determinada</p>
            ...
            <?php if (!empty($resultados)): ?>
                <div class="card">
                    ...
                    <p class="card-subtitle">Se encontraron <?php echo count($resultados); ?> registro(s) para este paciente</p>
