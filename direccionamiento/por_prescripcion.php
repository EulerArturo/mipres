...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['actualizar_estado'])) {
    $id_direccionamiento = $_POST['id_direccionamiento'] ?? '';
    $direccionado = isset($_POST['direccionado']) ? 1 : 0;
    $no_prescripcion = $_POST['no_prescripcion'] ?? '';
    
    try {
        $stmt = $pdo->prepare("INSERT INTO direccionamientos_estado (id_direccionamiento, no_prescripcion, direccionado, usuario_id) 
                               VALUES (?, ?, ?, ?) 
                               ON DUPLICATE KEY UPDATE direccionado = ?, fecha_actualizacion = CURRENT_TIMESTAMP");
        $stmt->execute([$id_direccionamiento, $no_prescripcion, $direccionado, $usuario['id'], $direccionado]);
        
        $mensaje = '✓ El estado del direccionamiento ha sido actualizado correctamente';
        $tipo_mensaje = 'success';
    } catch (PDOException $e) {
        $mensaje = 'No se pudo actualizar el estado. Por favor intente nuevamente o contacte al administrador';
        $tipo_mensaje = 'error';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['actualizar_estado'])) {
    ...
    if (empty($nit)) {
        ...
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', 'Validación fallida: NIT vacío');
        ...
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "Validación fallida: Token vacío. Prescripción: {$no_prescripcion}");
    } elseif (empty($no_prescripcion)) {
        $mensaje = 'Por favor ingrese el número de prescripción que desea consultar';
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "Validación fallida: Prescripción vacía. NIT: {$nit}");
    } else {
        ...
        $apiResult = $mipresApiClient->get(
            "DireccionamientoXPrescripcion/{$nit}/{$token_procesado}/{$no_prescripcion}",
            30
        );
        ...
        if ($curl_error) {
            $mensaje = 'No se pudo conectar con el servidor de MIPRES. Intente nuevamente';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "Error cURL. Prescripción: {$no_prescripcion}, HTTP: {$http_code}, Error: {$curl_error}");
            ...
            if (json_last_error() === JSON_ERROR_NONE && is_array($data)) {
                ...
                $mensaje = '✓ Consulta exitosa: Se encontraron ' . count($resultados) . ' registro(s) de direccionamiento para esta prescripción';
                $tipo_mensaje = 'success';

                registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO', "Consulta por prescripción: {$no_prescripcion}. Resultados: " . count($resultados));
            } else {
                $mensaje = 'La respuesta del servidor no tiene el formato esperado. Por favor intente nuevamente';
                $tipo_mensaje = 'error';
                registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "Respuesta JSON inválida. Prescripción: {$no_prescripcion}, HTTP: {$http_code}");
            }
        } elseif ($http_code === 401) {
            $mensaje = 'El token ha expirado o no es válido. Por favor genere un nuevo token temporal e intente nuevamente';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "HTTP 401. Prescripción: {$no_prescripcion}");
        } elseif ($http_code === 404) {
            $mensaje = 'No se encontraron registros de direccionamiento para el número de prescripción consultado';
            $tipo_mensaje = 'warning';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "HTTP 404 sin resultados. Prescripción: {$no_prescripcion}");
        } elseif ($http_code >= 500) {
            $mensaje = 'El servidor de MIPRES no está disponible en este momento. Por favor intente nuevamente en unos minutos';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "HTTP {$http_code}. Prescripción: {$no_prescripcion}");
        } else {
            $mensaje = "No se pudo completar la consulta. Si el problema persiste, contacte al soporte técnico (Código: {$http_code})";
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_ERROR', "HTTP {$http_code} no controlado. Prescripción: {$no_prescripcion}");
        }
    }
}
...
<head>
    ...
    <title>Direccionamiento por Prescripción - <?php echo APP_NAME; ?></title>
...
<body>
    ...
    <div class="layout">
        <aside class="sidebar">
            ...
            <div class="sidebar-section">
                ...
                <div class="sidebar-children open" id="menu-direccionamiento">
                    <a href="por_prescripcion.php" class="sidebar-item sidebar-child active">Por Num. Prescripción</a>
                    <a href="por_fecha.php" class="sidebar-item sidebar-child">Por Fecha</a>
                    <a href="por_paciente.php" class="sidebar-item sidebar-child">Por Paciente</a>
        ...
        <main class="main-content">
            ...
            <div class="card">
                <h2>Direccionamiento por Num. Prescripción</h2>
                <p class="card-subtitle">Consulta el direccionamiento de una prescripción específica</p>
                ...
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            name="token_temporal" 
                            required
                            placeholder="Ingrese el Token Temporal"
                            value="<?php echo htmlspecialchars(trim($_POST['token_temporal'] ?? $token_sesion, '"')); ?>"
                        >
                        <small>Token generado en la sección "Generación de Token"</small>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            required
                            placeholder="Ej: 20250702174001472796"
                            value="<?php echo htmlspecialchars($_POST['no_prescripcion'] ?? ''); ?>"
                        >
                    </div>
                ...
                <?php if (!empty($resultados)): ?>
                    ...
                    <?php foreach ($resultados as $index => $resultado): ?>
                        ...
                        <div class="result-card" data-direccionado="<?php echo $resultado['direccionado']; ?>">
                            ...
                            <!-- Checkbox para marcar como direccionado -->
                            ...
                            <div class="result-grid">
                                ...
                                <div class="result-field">
                                    ...
                                    <span><?php echo htmlspecialchars($resultado['IDDireccionamiento'] ?? 'N/A'); ?></span>
                                </div>
                                ...
                                <div class="result-field">
                                    <label>Consecutivo Tecnología:</label>
                                ...
                                <div class="result-field">
                                    <label>No. Entrega:</label>
                                    <span><?php echo htmlspecialchars($resultado['NoEntrega'] ?? 'N/A'); ?></span>
                                </div>
                                ...
                                <div class="result-field">
                                    <label>Código Municipio Entrega:</label>
                                ...
                                <div class="result-field">
                                    <label>Código Servicio Tecnología:</label>
                                    <span><?php echo htmlspecialchars($resultado['CodSerTecAEntregar'] ?? 'N/A'); ?></span>
                                </div>
                                
                                <div class="result-field">
                                    <label>Fecha Direccionamiento:</label>
                                    <span><?php echo htmlspecialchars($resultado['FecDireccionamiento'] ?? 'N/A'); ?></span>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
