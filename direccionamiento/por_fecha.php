...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nit)) {
        ...
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', 'Validación fallida: NIT vacío');
        ...
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "Validación fallida: Token vacío. Fecha: {$fecha}");
    } elseif (empty($fecha)) {
        $mensaje = 'Por favor ingrese la fecha para consultar el direccionamiento';
        $tipo_mensaje = 'error';
        registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "Validación fallida: Fecha vacía. NIT: {$nit}");
    } else {
        // Procesar token
        ...
        $apiResult = $mipresApiClient->get(
            "DireccionamientoXFecha/{$nit}/{$token_procesado}/{$fecha}",
            15
        );
        ...
        if ($curl_error) {
            ...
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "Error cURL. Fecha: {$fecha}, HTTP: {$http_code}, Error: {$curl_error}");
            ...
            if (json_last_error() === JSON_ERROR_NONE && is_array($data)) {
                ...
                $mensaje = '✓ Consulta exitosa: Se encontraron ' . count($resultados) . ' registro(s) de direccionamiento para esta fecha';
                $tipo_mensaje = 'success';

                registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA', "Consulta por fecha: {$fecha}. Resultados: " . count($resultados));
            } else {
                $mensaje = 'La respuesta del servidor no tiene el formato esperado. Por favor intente nuevamente';
                $tipo_mensaje = 'error';
                registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "Respuesta JSON inválida. Fecha: {$fecha}, HTTP: {$http_code}");
            }
        } elseif ($http_code === 401) {
            $mensaje = 'El token ha expirado o no es válido. Por favor genere un nuevo token temporal e intente nuevamente';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "HTTP 401. Fecha: {$fecha}");
        } elseif ($http_code === 404) {
            $mensaje = 'No se encontraron registros de direccionamiento para la fecha consultada';
            $tipo_mensaje = 'warning';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "HTTP 404 sin resultados. Fecha: {$fecha}");
        } elseif ($http_code >= 500) {
            $mensaje = 'El servidor de MIPRES no está disponible en este momento. Por favor intente nuevamente en unos minutos';
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "HTTP {$http_code}. Fecha: {$fecha}");
        } else {
            $mensaje = "No se pudo completar la consulta. Si el problema persiste, contacte al soporte técnico (Código: {$http_code})";
            $tipo_mensaje = 'error';
            registrar_log_actividad($pdo, $usuario['id'], 'CONSULTA_DIRECCIONAMIENTO_FECHA_ERROR', "HTTP {$http_code} no controlado. Fecha: {$fecha}");
        }
    }
}
...
<head>
    ...
    <title>Direccionamiento por Fecha - <?php echo APP_NAME; ?></title>
...
<body>
    ...
    <div class="layout">
        <aside class="sidebar">
            ...
            <div class="sidebar-section">
               ...
               <div class="sidebar-children open" id="menu-direccionamiento">
                    ...
                    <a href="por_fecha.php" class="sidebar-item sidebar-child active">Por Fecha</a>
        ...
        <main class="main-content">
            ...
            <div class="card">
                <h2>Direccionamiento por Fecha</h2>
                <p class="card-subtitle">Consulta el direccionamiento de una fecha específica</p>
